...
async function runSwiftProd(filename, components, { pluginId, isPlugin, isX, isSingleThread, extApis, transform, sourceMap, hookClass, uniModules, }) {
    ...
    (0, utils_1.genUTSPlatformResource)(filename, {
        ...
        hookClass,
        result,
        provider: (0, utils_1.resolveConfigProvider)('app-ios', pluginId, transform),
        uniModules,
    });
}
exports.runSwiftProd = runSwiftProd;
let isEnvReady = true;
async function runSwiftDev(filename, { components, isX, isSingleThread, isPlugin, extApis, transform, sourceMap, uniModules, }) {
    // 文件有可能是 app-android 里边的，因为编译到 ios 时，为了保证不报错，可能会去读取 android 下的 uts
    if (filename.includes('app-android')) {
        return;
    }
