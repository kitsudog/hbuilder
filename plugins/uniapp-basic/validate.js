...
function spawnPromise(command, args, options) {
	return new Promise((resolve, reject) => {
		...
		childProcess.stdout.on('data', (data) => {
			// var gbkStr = iconvlite.decode(data, "gbk");
...
var appleCertError = {
	passwordError: {
		"manifest.plus.distribute.apple.password": "私钥密码不正确"
	},
	errorP12: {
		"manifest.plus.distribute.apple.p12": "私钥证书不是有效的p12文件"
	},
	errorProvision: {
		"manifest.plus.distribute.apple.mobileprovision": "profile文件不是有效的mobileprovision文件"
	},
	errorAppid: {
		"manifest.plus.distribute.apple.appid": "AppID与profile文件不匹配, profile文件中AppID为 "
	},
	p12Expired: {
		"manifest.plus.distribute.apple.p12": "证书文件已过期，请重新生成证书"
	},
	p12Noteffective: {
		"manifest.plus.distribute.apple.p12": "证书文件尚未生效"
	},
	p12Mismatch: {
		"manifest.plus.distribute.apple.p12": "profile文件与私钥证书文件不匹配"
	},
	mobileprovisionMismatch: {
		"manifest.plus.distribute.apple.mobileprovision": "profile文件与私钥证书文件不匹配"
	}
}
		...
		for (var key in json) {
			if (key == "supportApsEnvironment") {
				if (appconfig && appconfig.permissions && appconfig.permissions.Push) {
					map.set("manifest.plus.distribute.apple.mobileprovision",
...
var androidCertError = {
	passwordError: {
		"manifest.plus.distribute.google.password": "私钥密码不正确"
	},
	keyPasswordError: {
		"manifest.plus.distribute.google.keypwd": "证书密码不正确"
	},
	storePasswordError: {
		"manifest.plus.distribute.google.storepwd": "证书库密码不正确"
	},
	Invalidkeystore: {
		"manifest.plus.distribute.google.keystore": "证书文件不是有效的keystore文件"
	},
	aliasnameError: {
		"manifest.plus.distribute.google.aliasname": "证书别名不正确"
	},
	keystoreExpired: {
		"manifest.plus.distribute.google.keystore": "证书文件已过期，请重新生成证书"
	},
	keystoreNoteffective: {
		"manifest.plus.distribute.google.keystore": "证书文件尚未生效"
	}
}
...
async function checkAndroidCert(request) {
	...
	var storePassword = request.androidStorePassword;
	...
	cmdArray.push(password);
	if (storePassword) {
		cmdArray.push("-androidStoreCertPassword");
		cmdArray.push(storePassword);
	}
	var result = spawnSync(java, cmdArray);
	var gbkStr = iconvlite.decode(result.stdout, "gbk");
	var str = gbkStr.toString();
...
async function getAlias(request) {
	var storePassword = request.androidStorePassword;
	var certFile = request.androidCertfile;
	var java = await hx.util.getProgram("java");
	if (util.isStringEmpty(java)) {
	...
	try {
		...
		if (platformIos(platform)) {
			...
			if (kernel && kernel.length > 0 && kernel.indexOf(ios) > -1) {
				...
				if ("UIWebview" == value) {
					if (!module || !module.uiWebview) { //permissions中没有配置以下UIWebview模块
						map.set("manifest.permissions.UIWebview",
							"应用配置默认使用UIWebview内核，需在“模块权限配置”页中勾选“iOS UIWebview”模块");
					}
				}
			}
			...
			if (iOSIcon) {
				...
				if (prerendered && typeof(prerendered) == 'string' && prerendered.toLowerCase() == "true" && prerendered
					.toLowerCase() == "false") {
					iOSIcon.prerendered = "false";
				}

				if (!util.isStringEmpty(iOSIcon.appstore)) {
					checkPNGFile(map, iOSIcon.appstore, project, "manifest.plus.distribute.icons.ios.appstore",
						markIconSplashChanged);
				}
				if (isIPhone) {
					var iphoneIconcheckArr = ["app@2x", "app@3x", "spotlight@2x",
						"spotlight@3x", "settings@2x", "settings@3x", "notification@2x", "notification@3x"
					];
					checkPNGFileOfFileds(map, iOSIcon.iphone, "manifest.plus.distribute.icons.ios.iphone.", project,
						markIconSplashChanged, iphoneIconcheckArr);
				}
				if (isIPad) {
					var ipadIconcheckArr = ["app", "app@2x", "proapp@2x", "spotlight",
						"spotlight@2x", "settings", "settings@2x", "notification", "notification@2x"
					]
					checkPNGFileOfFileds(map, iOSIcon.ipad, "manifest.plus.distribute.icons.ios.ipad.", project,
						markIconSplashChanged, ipadIconcheckArr);
				}
			}
			...
			if (iOSSplashScreen && iosStyle) {
				if ("default" == iosStyle.toLowerCase() || util.isStringEmpty(iosStyle)) {
					if (isIPhone) {
						checkPNGFileOfFileds(map, iOSSplashScreen.iphone,
							"manifest.plus.distribute.splashscreen.ios.iphone.", project, markIconSplashChanged);
					}
					if (isIPad) {
						checkPNGFileOfFileds(map, iOSSplashScreen.ipad,
							"manifest.plus.distribute.splashscreen.ios.ipad.", project, markIconSplashChanged);
					}
				}

				if ("storyboard" == iosStyle.toLowerCase()) {
					var storyboard = iOSSplashScreen.storyboard;
					checkStoryboardFile(map, storyboard, project,
						"manifest.plus.distribute.splashscreen.ios.storyboard");
				}
			}
		...
		if (platformAndroid(platform)) {
			...
			if (androidIcon) {
				...
				checkPNGFileOfFileds(map, androidIcon, "manifest.plus.distribute.icons.android.", project,
					markIconSplashChanged, androidIconcheckArr);
			}
			...
			if (androidSplashScreen && androidStyle) {
				if ("default" == androidStyle || util.isStringEmpty(androidStyle)) {
					checkPNGFileOfFileds(map, androidSplashScreen, "manifest.plus.distribute.splashscreen.android.",
						project, markIconSplashChanged);
				}
			}

			if (config) {
				var isunix = config["uni-app-x"];
				if (isunix) {
					var unixandroid = ["xhdpi", "xxhdpi", "xxxhdpi"];
					var splashScreensAndroid = config?.plus?.distribute?.splashScreens?.android;
					if (splashScreensAndroid) {
						for (var item of unixandroid) {
							if (splashScreensAndroid.hasOwnProperty(item)) {
								var filePath = splashScreensAndroid[item];
								if (util.isStringEmpty(filePath)) continue;
								var file = util.getFileExists(project.project, filePath);
								if (!file) {
									map.set("app.distribute.splashScreens.android." + item, "文件不存在");
								} else if (fs.statSync(file).size == 0) {
									map.set("app.distribute.splashScreens.android." + item, "不是合法的png文件，请检查是否为空白文件");
								}
							}
						}
					}
					var android12 = config?.plus?.distribute?.splashScreens?.android12;
					if(android12){
						var background = android12?.background;
						if(background){
							var regex = /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/;
							if(!regex.test(background)){
								map.set("app.distribute.splashScreens.android12.background", "请输入正确的颜色值");
							}
						}
						var brand = android12?.brand;
						if(brand){
							for(var item of unixandroid){
								if(brand.hasOwnProperty(item)){
									var filePath = brand[item];
									if (util.isStringEmpty(filePath)) continue;
									var file = util.getFileExists(project.project, filePath);
									if (!file) {
										map.set("app.distribute.splashScreens.android12.brand." + item, "文件不存在");
									} else if (fs.statSync(file).size == 0) {
										map.set("app.distribute.splashScreens.android12.brand." + item, "不是合法的png文件，请检查是否为空白文件");
									}
								}
							}
						}
						var icon = android12?.icon;
						if(icon){
							for(var item of unixandroid){
								if(icon.hasOwnProperty(item)){
									var filePath = icon[item];
									if (util.isStringEmpty(filePath)) continue;
									var file = util.getFileExists(project.project, filePath);
									if (!file) {
										map.set("app.distribute.splashScreens.android12.icon." + item, "文件不存在");
									} else if (fs.statSync(file).size == 0) {
										map.set("app.distribute.splashScreens.android12.icon." + item, "不是合法的png文件，请检查是否为空白文件");
									}
								}
							}
						}
			...
			var google = config?.plus?.distribute?.google;
			if (google) {
				var permissions = google?.permissions;
				var excludePermissions = google?.excludePermissions;
				var permissionJson;
				var excludePermissionsJson;
				const options = {
				...
				var excludePermissionsError = false
				
				var manifestPermissionsKey = "manifest.plus.distribute.android.permissions";
				var manifestExcludePermissionsKey = "manifest.plus.distribute.android.excludePermissions";
				if(project && (project.nature === "App" || project.nature === "Wap2App")){
					manifestPermissionsKey = "manifest.plus.distribute.google.permissions";
					manifestExcludePermissionsKey = "manifest.plus.distribute.google.excludePermissions";
				}
				
				try {
					if (permissions && permissions.length > 0) {
						var permissionsStr = permissions.join('');
						var isValid = XMLValidator.validate(permissionsStr, {
							allowBooleanAttributes: false
						});
						if(isValid && !isValid.err){
							permissionsStr = "<root>" + permissionsStr + "</root>";
							isValid = XMLValidator.validate(permissionsStr, {
								allowBooleanAttributes: false
							});
							if (isValid) {
								const parser = new XMLParser(options);
								let jObj = parser.parse(permissionsStr);
								permissionJson = jObj?.root;
							} else {
								// 校验失败
								map.set(manifestPermissionsKey, "格式不正确");
				...
				try {
					if (excludePermissions && excludePermissions.length > 0) {
						var pexcludePermissionssStr = excludePermissions.join('');
						var isValid = XMLValidator.validate(pexcludePermissionssStr, {
							allowBooleanAttributes: false
						});
						if(isValid && !isValid.err){
							pexcludePermissionssStr = "<root>" + pexcludePermissionssStr + "</root>";
							isValid = XMLValidator.validate(pexcludePermissionssStr, {
								allowBooleanAttributes: false
							});
							if (isValid) {
								const parser = new XMLParser(options);
								let jObj = parser.parse(pexcludePermissionssStr);
								excludePermissionsJson = jObj?.root;
							} else {
								// 校验失败
								map.set(manifestExcludePermissionsKey, "格式不正确");
								excludePermissionsError = true
							}
						}else{
							map.set(manifestExcludePermissionsKey, "格式不正确");
							excludePermissionsError = true
				...
				if (permissionJson && permissionJson["#text"]) {
					// 校验失败
					map.set(manifestPermissionsKey, "格式不正确");
					permissionsError = true
				}

				if (excludePermissionsJson && excludePermissionsJson["#text"]) {
					// 校验失败
					map.set(manifestExcludePermissionsKey, "格式不正确");
					excludePermissionsError = true
				}
				...
				if(!permissionsError  && permissionJson){
					var permissionPaths = convertToPaths(permissionJson);
					if(permissionPaths){
						for(var items of Object.values(permissionPaths)){
							if(Array.isArray(items)){
								if(isRepeat(items)){
									map.set(manifestPermissionsKey, "存在重复数据");
									permissionsError = true;
									break;
								}
							}
				...
				if(!excludePermissionsError  && excludePermissionsJson){
					var excludePermissionsPaths = convertToPaths(excludePermissionsJson);
					if(excludePermissionsPaths){
						for(var items of Object.values(excludePermissionsPaths)){
							if(Array.isArray(items)){
								if(isRepeat(items)){
									map.set(manifestExcludePermissionsKey, "存在重复数据");
									excludePermissionsError = true
									break;
								}
							}
				...
				function convertToPaths(obj, path = '', result = {}) {
						...
						if (Array.isArray(value)) {
							value.forEach((item) => {
								if (item !== null && typeof item === 'object') {
									convertToPaths(item, currentPath, result);
								}
							});
						} else if (typeof value === 'object' && value !== null) {
							convertToPaths(value, currentPath, result);
						} else {
				...
				if (!permissionsError && !excludePermissionsError && permissionJson && excludePermissionsJson) {
					var permissionPaths = convertToPaths(permissionJson);
					var excludePermissionsPaths = convertToPaths(excludePermissionsJson);

					var isSame = false
					for (var permissionKey in permissionPaths) {
						var permissionValue = permissionPaths[permissionKey];

						if (excludePermissionsPaths.hasOwnProperty(permissionKey)) {
							var excludePermissionsValue = excludePermissionsPaths[permissionKey];
							for (var item of permissionValue) {
								if (excludePermissionsValue.indexOf(item) > -1) {
									map.set(manifestPermissionsKey, "与强制移除的权限存在重复数据");
									map.set(manifestExcludePermissionsKey,"与额外添加的权限存在重复数据");
									isSame = true;
									break;
								}
							}
			...
			if (plugins) {
				...
				if (platformAndroid(platform)) {
					if (hasgoogleStatics && util.isStringEmpty(googleStaticsandroidConfig)) {
						map.set("manifest.plus.distribute.statics.google.config_android",
							"Google Analytics for Firebase的config_android不能为空");
					}
				}

				if (platformIos(platform)) {
					if (hasgoogleStatics && util.isStringEmpty(googleStaticsiosConfig)) {
						map.set("manifest.plus.distribute.statics.google.config_ios",
							"Google Analytics for Firebase的config_ios不能为空");
					}
				}
			}
		}
...
function checkPluginOfFileds(errors, sdkObject, sdkNodes, sdkName, moduleNodes, hasModule, checkUniqueness,
	checkMissPlugin, platform, sdkProperties) {
	var pluginsNodes = [];
	if (sdkObject) {
			...
			try {
				if (pluginObject && typeof(pluginObject) != 'string') {
					...
					filterFields(pluginObject, pluginNodes, function(fieldNodes, fieldValue) {
						if (fieldNodes.endsWith(".description") || fieldNodes.endsWith(
								"__platform__")) {
							return;
						} else if (fieldNodes.endsWith("_ios") && (platform.toUpperCase() ==
								"ANDROID")) {
							//android平台不校验ios相关属性
							return;
							...
							if (supportPlatforms.indexOf("ios") > -1 && fieldNodes.endsWith(
									".payment.stripe.returnURL_ios")) {
								errors.set(fieldNodes, "");
							}

							if (supportPlatforms.indexOf("ios") > -1 && fieldNodes.endsWith(
									".payment.paypal.returnURL_ios")) {
								errors.set(fieldNodes, "");
							}

							if (supportPlatforms.indexOf("android") > -1 && fieldNodes.endsWith(
									".payment.paypal.returnURL_android")) {
								errors.set(fieldNodes, "");
							}
		...
		if (!file) {
			...
			try {
				...
				zip.getEntries().forEach(function(entry) {
					...
					if (entryName.startsWith("__MACOSX") || entryName.startsWith(".git") || entryName
						.startsWith(".svn")) {
						return;
					}
				...
				if (!hasStoryboardFile) {
					errors.set(fieldPath,
						"不是合法的storyboard文件，请确保选择的zip包内存在storyboard文件,如何制作storyboard请参考<a style='color:#298bdb' href='https://ask.dcloud.net.cn/article/37475'>文档</a>"
					);
				}
			} catch (e) {
...
const compareImageConfig = {
	"manifest.plus.distribute.icons.ios.appstore": "1024x1024"
}

function compareImageSize(filePath, fieldNodes) {
	if (compareImageConfig && compareImageConfig[fieldNodes]) {
		var size = compareImageConfig[fieldNodes];
...
function checkPNGFile(errors, filePath, project, fieldNodes, markIconSplashChanged) {
	try {
		...
		if (!file) {
			...
			let newByte = fs.readSync(fd, byte, {
				offset: 0,
				length: 8
			});
			if (fd > 0) {
				...
				var pngSize = compareImageSize(file, fieldNodes);
				if (pngSize) {
					errors.set(fieldNodes, `图标分辨率不正确，当前配置图标分辨率为 ${pngSize.width}x${pngSize.height}，要求图标分辨率为${compareImageConfig[fieldNodes]}`)
				}
			}
		}
	} catch (e) {
...
module.exports = {
	...
	verifyAndroidCert: checkAndroidCert,
	getAlias
}
